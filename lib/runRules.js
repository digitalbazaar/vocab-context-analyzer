 ...
 */
import {brokenHierarchy} from './rules/brokenHierarchy.js';
import {contextIriResolves} from './rules/contextIriResolves.js';
import {coverage} from './rules/coverage.js';
import {deprecatedTerm} from './rules/deprecatedTerm.js';
import {domainRange} from './rules/domainRange.js';
import {iriCollision} from './rules/iriCollision.js';
import {missingCoercion} from './rules/missingCoercion.js';
import {orphanMapping} from './rules/orphanMapping.js';
import {protectedContext} from './rules/protectedContext.js';
import {stableIri} from './rules/stableIri.js';
import {termHasDefinition} from './rules/termHasDefinition.js';
import {unsafeVocab} from './rules/unsafeVocab.js';
...
export function runRules(model) {
  const findings = [];
  // findings the shell pre-computed for checks that need IO/processing (e.g.
  // the ctx/invalid-jsonld smoke test), passed through so the core stays pure
  if(Array.isArray(model.preFindings)) {
    findings.push(...model.preFindings);
  }
