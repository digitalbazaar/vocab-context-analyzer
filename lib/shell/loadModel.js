...
export async function loadModel({vocab, context, documentLoader} = {}) {
  ...
  const preFindings = await _smokeTestContext(context, loader);
  if(preFindings.length > 0) {
    model.preFindings = preFindings;
  }
  return model;
}

// the JSON-LD validity / canonicalization smoke test (SPEC section 5.1). Unlike
// the pure rules, it needs jsonld processing, so the shell runs it here and
// surfaces any failure as a pre-computed finding the core passes through.
async function _smokeTestContext(context, loader) {
  try {
    await jsonld.expand(context, {documentLoader: loader});
    return [];
  } catch(e) {
    return [createFinding({
      id: 'ctx/invalid-jsonld',
      severity: SEVERITY.error,
      artifact: ARTIFACT.context,
      message: `Context does not process as valid JSON-LD: ` +
        `${e.details?.code ?? e.message}.`,
      remediation: 'Fix the JSON-LD syntax error in the context.'
    })];
  }
}
