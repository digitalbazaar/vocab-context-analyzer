...
export async function resolveContext(contextDocument, {documentLoader} = {}) {
  ...
  const prefixes = {};
  const terms = [];
  _collect(context, prefixes, terms);
...
// terms keep their own definition for resolution.
function _collect(context, prefixes, terms) {
  ...
  for(const [key, value] of Object.entries(context)) {
    ...
    if(value !== null && typeof value === 'object' && '@context' in value) {
      _collect(value['@context'], prefixes, terms);
    }
  }
}
