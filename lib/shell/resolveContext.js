...
export async function resolveContext(contextDocument, {documentLoader} = {}) {
  ...
  for(const {term, definition} of terms) {
    const {iri, coercion, container} = await _resolveTerm(
...
// context, returning {iri, coercion, container}. The active context is built
// from the prefixes plus this single term, so a malformed sibling term
// elsewhere in the context cannot blind this one. Returns iri null (and no
// coercion/container) when the term cannot be processed even in isolation.
async function _resolveTerm({prefixes, term, definition, loader}) {
  ...
  try {
    ...
    return {iri: null, coercion: undefined, container: undefined};
  }
  ...
  // @container is normalized by jsonld to an array (e.g. ["@language"]); it
  // distinguishes intentional language-map / collection variants of a term
  // from a genuine duplicate alias (see ctx/iri-collision).
  const container = Array.isArray(entry['@container']) ?
    entry['@container'] : undefined;
  return {iri: entry['@id'], coercion, container};
}
