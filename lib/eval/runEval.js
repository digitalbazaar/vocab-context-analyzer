 ...
 * citations must all be valid, the deferral rate must be 0, and every
 * clean-labeled case (no expected rules) must be finding-free. Faithfulness is
 * only meaningful once renderings exist, so it gates only when renderings are
 * supplied. The LLM-dependent checks pass trivially while no LLM findings
 ...
 *   faithfulness, cleanliness, hardGatePassed}`.
 */
export function evaluate({cases, findingsByCase}) {
  ...
  return {recall, citation, deferral, faithfulness, cleanliness,
    hardGatePassed};
}
