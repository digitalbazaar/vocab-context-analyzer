...
const REQUIRED_STRINGS = ['name', 'context'];
// `vocab` is optional: generated golden cases pair a vocabulary with a context,
// while a context-only anchor omits it (the model is built from the context
// alone). `description` documents what a case exercises; both optional strings.
const OPTIONAL_STRINGS = ['vocab', 'description', 'labeledBy', 'labeledAt'];
const ALLOWED_KEYS = new Set([
  'name', 'vocab', 'context', 'expectedRuleIds', 'exact', 'description',
...
// the keys of a standalone label payload (see `validateLabel`)
const LABEL_KEYS = new Set(['overall', 'designRank', 'subjectiveIssues']);
...
// `exact` (regression anchors) requires the produced findings to match
// `expectedRuleIds` exactly; a boolean when present.
function _validateExact(value) {
  if(value !== undefined && typeof value !== 'boolean') {
    throw new TypeError('Manifest entry "exact" must be a boolean.');
  }
}
...
/**
 * Validate a standalone per-case label — the labeling app's save/import
 * payload shape, where `null` (not absence) marks an unset field. Throws a
 * `TypeError` naming the case on the first violation, so a bad payload is
 * rejected before it is written anywhere.
 *
 * @param {object} label - The label: `{overall, designRank,
 *   subjectiveIssues}`.
 * @param {string} name - The case name, for error messages.
 *
 * @returns {object} The validated label.
 */
export function validateLabel(label, name) {
  if(label === null || typeof label !== 'object') {
    throw new TypeError(`Label for "${name}" must be an object.`);
  }
  for(const key of Object.keys(label)) {
    if(!LABEL_KEYS.has(key)) {
      throw new TypeError(
        `Label for "${name}" has unexpected key "${key}".`);
    }
  }
  try {
    _validateOverall(label.overall ?? undefined);
    _validateDesignRank(label.designRank ?? undefined);
    if(!Array.isArray(label.subjectiveIssues)) {
      throw new TypeError(
        'Manifest entry "subjectiveIssues" must be an array.');
    }
    _validateSubjectiveIssues(label.subjectiveIssues);
  } catch(e) {
    throw new TypeError(`Label for "${name}": ${e.message}`);
  }
  return label;
}
